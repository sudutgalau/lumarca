...
//			minY = LumarcaPRO.WIN_HEIGHT/5.5f;
//			maxY = LumarcaPRO.WIN_HEIGHT/6.0f;

		for (int i = 0; i < lineNum; i++) {

			if (LumarcaPRO.WIN_HEIGHT - map[i] > maxY) {
				maxY = LumarcaPRO.WIN_HEIGHT - map[i];
			}

			if (LumarcaPRO.WIN_HEIGHT - map[i] < minY) {
				minY = LumarcaPRO.WIN_HEIGHT - map[i];
			}
		}
...
//		minY = 10;//-LumarcaPRO.WIN_HEIGHT;
//		maxY = LumarcaPRO.WIN_HEIGHT/2.0f;
		...
		for (int lineCount = 0; lineCount < lineNum; lineCount++) {
			...
			float camDiff = PApplet.dist(
					...
					projectorY,
					projectorZ, 
					lineCount * (LumarcaPRO.WIN_WIDTH/lineNum), 
					minY,
					LumarcaPRO.WIN_HEIGHT);

//			System.out.println("camDiff: " + camDiff);
			
			float yIntersect = 
				(projectorY - minY) / 
				((projectorY - LumarcaPRO.WIN_HEIGHT + map[lineCount]) / camDiff);
			

//			float yIntersect = PApplet.sqrt(
//				PApplet.pow(projectorY - ((projectorY - LumarcaPRO.WIN_HEIGHT + map[lineCount]) / camDiff), 2) +
//				PApplet.pow(projectorY - ((projectorY - LumarcaPRO.WIN_HEIGHT + map[lineCount]) / camDiff), 2));
				
			
//			float f = (projectorX - (lineCount * (LumarcaPRO.WIN_WIDTH/lineNum)) / camDiff) * yIntersect;
			
//			coords[lineNum] = new Coord(
//					(projector_x - (lineNum * 4 + 0)) / camDiff,
//					(projector_y - LumarcaPRO.WIN_HEIGHT + map[lineNum]) / camDiff,
//					(projector_z - 0f) / camDiff, yIntersect, camDiff);

			
			lines[lineCount] = new Line(
					new Coord(
							projectorX - ((projectorX - (lineCount * (LumarcaPRO.WIN_WIDTH/lineNum))) / camDiff) * yIntersect,
							minY,
							projectorZ - (projectorZ / camDiff) * yIntersect, 
							yIntersect, camDiff), 
					new Coord(
							projectorX - ((projectorX - (lineCount * (LumarcaPRO.WIN_WIDTH/lineNum))) / camDiff) * yIntersect,
							minY - LumarcaPRO.WIN_HEIGHT/3,
							projectorZ - (projectorZ / camDiff) * yIntersect));
	...
	private void makeMap(String fileName) {
		...
		for (int i = 0; i < map.length; i++) {
			map[i] = LumarcaPRO.WIN_HEIGHT - pApplet.random((LumarcaPRO.WIN_HEIGHT/2) * 2/3, 
					LumarcaPRO.WIN_HEIGHT/2);
...
//		}
	...
	public void drawCalebrate() {
		...
		for (int i = 0; i < lineNum; i++) {
			...
			// config lines
			pApplet.line(i * 4 + 0, 0, i * 4 + 0, LumarcaPRO.WIN_HEIGHT);

			pApplet.noStroke();

			// albert's points
			pApplet.ellipse(i * 4 + 0, LumarcaPRO.WIN_HEIGHT - map[i], 8, 8);
		...
		pApplet.line(0, minY, LumarcaPRO.WIN_WIDTH, minY);
		pApplet.line(0, maxY, LumarcaPRO.WIN_WIDTH, maxY);
	}
...
//		gl.glEnd();
		...
		gl.glBegin(GL.GL_LINE_STRIP);
		gl.glVertex3f(
				 + ((coord.x - projectorX)/coord.yIntersect) * coord.camDiff + projectorX - 8f,
				 + ((coord.y - projectorY)/coord.yIntersect) * coord.camDiff + projectorY,
				 + ((coord.z - projectorZ)/coord.yIntersect) * coord.camDiff + projectorZ);
		gl.glVertex3f(
				 + ((coord.x - projectorX)/coord.yIntersect) * coord.camDiff + projectorX + 8f,
...
//		gl.glVertex3f(LumarcaPRO.WIN_WIDTH - 10f, minY, 
//				((coord.z - projectorZ)/coord.yIntersect) * coord.camDiff + projectorZ);
//		
//		gl.glVertex3f(10f, maxY, 0f);
//		gl.glVertex3f(LumarcaPRO.WIN_WIDTH - 10f, maxY,  
//				((coord.z - projectorZ)/coord.yIntersect) * coord.camDiff + projectorZ);
...
//		System.out.println((coord.z/coord.camDiff)*coord.yIntersect);
	...
	public void drawVertLines(GL gl, int lineNum) {
		...
		Util.drawLine(gl, 
				...
				new Coord(
						coord.x,
						coord.y - LumarcaPRO.WIN_HEIGHT/3,
						coord.z),
				new Coord(
						coord.x,
						coord.y,
